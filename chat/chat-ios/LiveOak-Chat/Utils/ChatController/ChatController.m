...
- (void) topRightPressed {
    if ([(NSObject *)_delegate respondsToSelector:@selector(rightButtonClicked:)]) {
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!message[kMessageSize]) {
        NSString * content =  [NSString stringWithFormat:@"%@: %@", message[kMessageUser], message[kMessageContent]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView
    ...
    if (!message[kMessageRuntimeSentBy]) {
         ...
         */
         ...
         if ([_currentUserId isEqualToString:message[kMessageUser]]) {
...
- (void) setMessagesArray:(NSArray *)messagesArray {
    // create mutable objects  copy of the containing nsdictionary objects
    // since chatcontroller modifies them and adds calculated cell size
    if (messagesArray) {
        _messagesArray = [NSPropertyListSerialization propertyListWithData:[NSPropertyListSerialization dataWithPropertyList:messagesArray format:NSPropertyListBinaryFormat_v1_0 options:0 error:nil]
                                                              options:NSPropertyListMutableContainersAndLeaves format:NULL error:nil];
    }
